       ...
       var queryRequest =[email]
                         ...
                         if (queryResponse.error == 'false') {
                            ...
                            if(queryResponse.result.length>0)
                            {
                                ...
                                {
                                    var token = jwt.sign({email:req.body.email,id:queryResponse.result[0].id}, 'mysecret');
                                    console.log(token)  
                                    resolve({status:200,id:queryResponse.result[0].id,message:"Login Successfull",error:"false"})

                                }
                                else{
                                resolve({status:401,message:"Invalid Email id or password",error:"true"})
                            }
                            }
                            else{
                                resolve({status:401,message:"Invalid Email id or password",error:"true"})
